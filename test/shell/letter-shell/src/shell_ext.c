 ...
 */
static unsigned long long shellExtParseNumber(char *string)
{
    ShellNumType type = NUM_TYPE_DEC;
    char radix = 10;
    char *p = string;
 ...
 */
unsigned long long shellExtParsePara(Shell *shell, char *string)
{
    if (*string == '\'' && *(string + 1))
    {
    ...
    {
        return (unsigned long long)shellExtParseNumber(string);
    }
...
{
    unsigned long long params[SHELL_PARAMETER_MAX_NUMBER] = {0};
